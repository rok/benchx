"""Static Arrow schemas and table conversion for measurement results."""
...
def _is_json_type(data_type: pa.DataType) -> bool:
...
def _field(
    ...
    *,
    ...
    return pa.field(
        ...
        data_type,
        nullable=nullable,
        metadata=JSON_METADATA if _is_json_type(data_type) else None,
    )
...
_MEASUREMENT_STORAGE_SCHEMA = pa.schema(
...
def validated_messages_to_arrow_table(
    messages: list[dict[str, Any]],
) -> pa.Table:
    """Convert validated messages to the static Arrow schema."""
    rows = [
        {
    ...
    table = pa.Table.from_pylist(
        rows,
        schema=_MEASUREMENT_STORAGE_SCHEMA,
    )
    return table.cast(MEASUREMENT_ARROW_SCHEMA)


def validated_message_to_arrow_table(
    message: dict[str, Any],
) -> pa.Table:
    """Convert one validated message to the static Arrow schema."""
    return validated_messages_to_arrow_table([message])


def write_validated_measurement_parquet(
    message: dict[str, Any],
    output: str | Path,
    *,
    message_schema_sha256: str,
) -> None:
    """Convert one validated message and write it to Parquet."""
    table = validated_message_to_arrow_table(message)
    metadata = dict(table.schema.metadata or {})
    metadata[b"benchx.message_schema_sha256"] = (
        message_schema_sha256.encode("ascii")
    )
    pq.write_table(
