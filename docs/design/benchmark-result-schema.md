...
- Observations are finite values in the quantity's unit after declared normalization. A numeric array is the common batch; structured observations carry per-observation ordinal, slot, group, pair, inner iteration count, inclusion, or exclusion data. When the inner iteration count varies within a batch, as with adaptive samplers that grow it per sample, each structured observation carries its own `inner_iterations` and `procedure.inner_iterations` is omitted.
...
| `inner_iterations` | procedure | Inner iterations actually used per observation, when the whole batch shares one count; a varying count is on each structured observation | `100` |
